import { Edit, Edit2, SpaceIcon, Sparkles } from "lucide-react";
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-6 text-slate-700">
      ...
      className="w-full max-w-lg p-6 bg-white rounded-xl border border-gray-200">
        ...
        <div className="mt-3 flex gap-3  flex-wrap sm:max-w-9/11 ">
          {articleLength.map((item,index)=><span onClick={()=>setSelectedLength(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-200 text-color-700 ' : 'text-gray-500 border-gray-300'}`} key={index}>{item.text }</span>)}
        </div>
