    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-6 text-slate-700">
      ...
      className="w-full max-w-lg p-6 bg-white rounded-xl border border-gray-200">
        ...
        <div className="mt-3 flex gap-3  flex-wrap sm:max-w-9/11 ">
          {blogCategories.map((item)=><span onClick={()=>setSelectedCategory(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedCategory === item ? 'bg-purple-200 text-purple-700 ' : 'text-gray-500 border-gray-300'}`} key={item}>{item}</span>)}
        </div>
